...
Deno.serve(async (req) => {
  ...
  if (code) {
    ...
    const tokenData = await tokenRes.json().catch(() => ({})) as { refresh_token?: string; error?: string };
    ...
    return htmlPage(
      ...
      `
      <h1>Google Drive OAuth</h1>
      <p>Copy the refresh token below and add it to Supabase secrets as <code>GOOGLE_DRIVE_REFRESH_TOKEN</code>:</p>
      <textarea readonly style="width:100%;height:80px;font-family:monospace;font-size:12px;padding:8px;" onclick="this.select()">${tokenData.refresh_token}</textarea>
  ...
  return htmlPage(
    ...
    <p>Click below to authorize TrendWatcher to upload creatives to your Drive.</p>
    ...
    <p style="color:#666;font-size:14px;">Use the account that will receive the uploaded images (e.g. your Workspace account with sdg-lab-shared).</p>
  `,
  );
});
